...
.ext-task:not(.has-ended) .end {
  ...
  function getDuration(startOrDuration, end = undefined) {
    const d = end !== undefined ? end.getTime() - startOrDuration.getTime() : startOrDuration;
    const duration = d / 1000 / 60/ 60;
    return duration.toFixed(2);
  }

  function formatDuration(startOrDuration, end = undefined) {
    let duration = end !== undefined ? end.getTime() - startOrDuration.getTime() : startOrDuration;
    let retstr = '';
  ...
  const summary = create('div', { class: 'ext-summary', children: [
  ...
  const buttonrow = create('div', { class: 'ext-main-row', children: [
    create('button', { text: '▶️ Start', class: 'ext-button-main', name: 'start-button' }),
    summary,
    create('div', { name: 'apply-wrapper', children: [
      create('a', { name: 'apply-hours', href: '#apply-hours', text: 'apply hours', class: 'ext-link' }),
      create('a', { name: 'apply-time', href: '#apply-time', text: 'apply times', class: 'ext-link' }),
      create('a', { name: 'apply-both', href: '#apply', text: 'apply both', class: 'ext-link' }),
      create('a', { name: 'clear', href: '#clear', text: 'clear', class: 'ext-link-danger' }),
    ] }),
    create('a', { name: 'clear', href: '#clear', text: 'clear', class: 'ext-link-danger' }),
  ] });
  ...
  function renderTasks() {
    while (tasksrow.childElementCount < tasks.length) {
      tasksrow.append(create('div', { class: 'ext-task', children: [
        create('span', { text: 'Start:' }),
        create('time', { name: 'start' }),
        create('span', { text: 'End:', class: 'end' }),
        create('time', { name: 'end', class: 'end' }),
        create('span', { text: 'Duration:' }),
        create('span', { name: 'duration' }),
        create('div', { class: 'running', name: 'stop-wrapper', children: [
          create('button', { name: 'stop-button', text: '⏹️ Stop', class: 'ext-button-secondary' }),
        ] }),
        create('a', { name: 'remove', href: '#remove', title: 'remove', text: '❌', class: 'ext-link-danger' }),
      ] }));
    }
    while (tasksrow.childElementCount > tasks.length) tasksrow.lastElementChild.remove();
    ...
    for (let i = 0; i < tasks.length; ++i) {
      ...
      setContent(taskel.querySelector('[name="duration"]'), formatDuration(task.start, task.end ? task.end : new Date()));
  ...
  function renderSummary() {
    ...
    setContent(summary.querySelector('[name="duration"]'), formatDuration(total.duration));
  }

  function renderMain() {
    wrap.classList.toggle('has-running-tasks', tasks.some(e => !('end' in e)));
    wrap.classList.toggle('has-finished-tasks', tasks.some(e => ('end' in e)));
  ...
  function calculateTotal() {
    ...
    for (let i = 0; i < tasks.length; ++i) {
      ...
      if (total.start === undefined || tasks[i].start.getTime() < total.start.getTime()) total.start = tasks[i].start;
      if (total.end === undefined || tasks[i].end.getTime() < total.end.getTime()) total.end = tasks[i].end;
  ...
  async function save(data = undefined) {
    ...
    await GM.setValue('tasks', data.map(t => ({
      ...t,
  ...
  tasksrow.addEventListener('click', function (ev) {
    const taskel = ev.target.closest('.ext-task');
    if (!taskel) return;
    ev.preventDefault();
    if (ev.target.name === 'remove') {
      tasks.splice(+taskel.dataset.task, 1);
    } else if (ev.target.name === 'stop-button') {
  ...
  buttonrow.addEventListener('click', function (ev) {
    ev.preventDefault();
    if (ev.target.name === 'start-button') {
      tasks.push({ start: new Date() });
      save();
    } else if (ev.target.name === 'clear') {
      tasks.splice(0);
      save();
    } else if (ev.target.name === 'apply-hours') {
      calculateTotal();
      document.getElementById('recording_hours').value = getDuration(total.duration);
    } else if (ev.target.name === 'apply-time') {
      calculateTotal();
      document.getElementById('workingtime_start').value = `${total.start.getHours()}:${formatNumber(total.start.getMinutes())}`;
      document.getElementById('workingtime_end').value = `${total.end.getHours()}:${formatNumber(total.end.getMinutes())}`;
    } else if (ev.target.name === 'apply-both') {
      calculateTotal();
      document.getElementById('recording_hours').value = getDuration(total.duration);
      document.getElementById('workingtime_start').value = `${total.start.getHours()}:${formatNumber(total.start.getMinutes())}`;
      document.getElementById('workingtime_end').value = `${total.end.getHours()}:${formatNumber(total.end.getMinutes())}`;
    } else { return; }
    if (['apply-hours', 'apply-time', 'apply-both'].indexOf(ev.target.name) !== -1) {
